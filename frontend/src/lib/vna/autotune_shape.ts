...
export async function shapeTune(probe: ShapeProbe, start: { tune: number; load: number }, opts: ShapeOpts): Promise<ShapeResult> {
  ...
  // Land on the best point and LOCK it with a few ±1 clicks (fast, predictable, can't diverge). The
  // joint Gauss-Newton fine stage was REVERTED (v0.8.1): its Broyden Jacobian didn't match the real
  // hardware, so from a big detune it wandered tune 28-43 / load 18-71 for 64 s and ended at −6 dB.
  let cur = await probe(best.tune, best.load);
  best = { tune: best.tune, load: best.load, cost: costAt(cur) };
  for (let k = 0; k < 10 && !convergedAt(cur) && !stop(); k++) {
    let moved = false;
